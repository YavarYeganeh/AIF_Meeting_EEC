...
MTTF = [3600]  # Avg. time to failure [sec] of machines in the workstation # Alberto recommended to adjust this
MTTR = [30]  # Time it takes to repair a machine [sec] in the workstation # Alberto recommended to adjust this 
...
W_STANDBY = [1.5]  # Consumption [kW] for standby state of machines in each workstation

ALPHA = 0.97 # shouldn't be 1 or 0 as those need some adjustments in the code see Alberto's decision maker
...
class Prod_System:  # This is the environment with whom the agent interacts
    ...
    def reward(self):
        """ New reward/preference function that considers consumption rate and throughput of a specified past period compared to the max theoretical ones. This specific function is linear response to throughput and average consumption"""    
    ...
    def raw_performance(self):
        ...
        if len(self.parts_made_buffer) > 1 and len(self.consumption_tot_buffer) > 1:  # Avoiding empty buffer
    ...
    def performance(self, reference_throughput=3.0011750114655458, reference_consumption=79.29417007089512,  reference_energy_consumption_per_part=1586.6030997276823):
        # Calculate metrics
